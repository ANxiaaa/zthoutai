<template>
		...
		<el-dialog title="表格显示列" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
			<el-table ref="fitlerTable" :data="columns" max-height="500px" height="auto" tooltip-effect="dark" header-align="left" align="left" :size="size" style="width: 100%" @selection-change="selectionChange">
            ...
            <el-input :size="size" v-model="scope.row.label"></el-input>
				...
				<el-table-column label="最小宽度">
					<template slot-scope="scope">
            <el-input :size="size" v-model= "scope.row.minWidth" :disabled="scope.row.minWidth === 0"></el-input>
          </template>
				</el-table-column>
			</el-table>
      ...
      setTimeout(() => {
        if(this.first){
          ...
          for(let i = 0;i< this.columns.length;i ++){
            this.$refs.fitlerTable.toggleRowSelection(this.columns[i],true)
          }
        }
      }, 0);
    },
