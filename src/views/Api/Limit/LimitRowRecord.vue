<template>
  <div class="page-container">
	...
	<kt-table :showBatchDelete="false" :data="pageResult" @handleCurrentChange="handleCurrentChange" :columns="filterColumns" @findPage="findPage" @handleEdit="handleEdit" @handleDelete="handleDelete">
		<kt-button icon="el-icon-search" slot="ktbtna" slot-scope="scope" label="查看" :key="scope.row" perms="limit:row:record:view" size="mini" type="primary" @click="bureauView" :loading="viewLoading"/>
	</kt-table>
	<!-- 显示页面 -->
...
export default {
	...
	methods: {
		...
		findPage: function (data) {
			...
			this.$api.limitRowCity.recordFindPage(this.pageRequest).then((res) => {
                console.log(res)
...
      	},
		initColumns : function(){
			this.columns = [
				...
				{prop:"lpNum", label:"车牌号", minWidth:100},
