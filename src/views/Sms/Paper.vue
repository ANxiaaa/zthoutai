<template>
    <div class="container">
            ...
            <div id="tab">
                ...
                :data="fansData" height="598">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <kt-button icon="fa fa-edit" label="查看" perms="sms:manager:view" :loading="viewLoading" @click="handleEdit(scope.row)"/>
                            <kt-button v-if="scope.row.enable" icon="fa fa-close" label="禁用" perms="sms:temp:enable" type="danger" @click="handleDelete(scope.row)"/>
                            <kt-button v-else icon="fa fa-check" label="解除" perms="sms:temp:enable" type="success" @click="handleDelete(scope.row)"/>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
		...
		<el-dialog :title="operation?'设置':'查看'" :modal="false" :visible.sync="dialogVisible" :close-on-click-modal="false">
            <div id="tmpview">
                <el-form :model="managerData" label-width="80px"  ref="dataForm" label-position="right">
                    <el-form-item v-for="(value, key) in manager" :key="key" :label="value" prop="id">
                        <el-input v-model="managerData[key]" :disabled="manageredit" auto-complete="off"></el-input>
                    </el-form-item>
                </el-form>
            </div>
			<div slot="footer" class="dialog-footer">
				<kt-button v-if="manageredit" label="修改" perms="sms:manager:edit" type="primary" @click="manEdit"/>
				<kt-button v-else label="保存" perms="sms:manager:edit" type="primary" @click="saveman"/>
...
      	},
        ...
        findPage(data, find){
            ...
            this.$api.sms.findPage(data).then(res=>{
                console.log(res)
